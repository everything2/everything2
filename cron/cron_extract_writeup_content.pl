...
#
# Output files in S3:
#   - writeup-content-full.json.gz    (all writeups, compressed)
...
my $recent_data = extract_writeups($recent_sql, [$writeup_type_id, $date_30_days_ago]);
my $recent_json = $json->encode({
    extracted_at => $timestamp,
    type => 'recent_30_days',
...
# ============================================================================
# 3. Full export (all writeups, gzipped) - for comprehensive analysis
# ============================================================================
print $APP->commonLogLine("Extracting full writeup corpus");
...
my $full_data = extract_writeups($full_sql, [$writeup_type_id]);
my $full_json = $json->encode({
    extracted_at => $timestamp,
    type => 'full_export',
    count => scalar(@{$full_data->{writeups}}),
    writeups => $full_data->{writeups}
});

# Compress full export
my $compressed;
gzip \$full_json => \$compressed
    or die "gzip failed: $GzipError\n";

print $APP->commonLogLine("Uploading full export (" . length($compressed) . " bytes compressed)");
$s3->upload_data("writeup-content-full.json.gz", $compressed, { content_type => "application/gzip" });
...
my $manifest = {
    ...
    files => {
        ...
        'writeup-content-full.json.gz' => {
            description => 'Full writeup corpus (gzip compressed)',
            count => scalar(@{$full_data->{writeups}}),
            compressed => 1,
            size_bytes => length($compressed),
            uncompressed_size => length($full_json)
        }
    },
    statistics => {
        total_writeups => scalar(@{$full_data->{writeups}}),
        with_links => count_with_links($full_data->{writeups}),
        average_length => average_length($full_data->{writeups})
    }
};
...
print $APP->commonLogLine("Export complete. Total writeups: " . scalar(@{$full_data->{writeups}}));
