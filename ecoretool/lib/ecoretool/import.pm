...
sub _inputs
{
	...
	{
		"default" => "",
		"help" => "Don't actually install the nodes inside the nodepack, just do that diffing",
		"default" => 0,
	},
	};
}
...
sub diffable_var_string
{
	...
	{
		# Direct from node XML construct
		foreach my $item(@$vars)
		{
			$varhash->{$item->{key}} = $item->{value};
		}
	}else{
		$varhash = {Everything::getVarHashFromStringFast($vars)};
	}

	my $outstr = "";
	foreach my $key(sort { $b cmp $a } keys %$varhash)
	{
		$varhash->{$key} = "(undef)" if not defined($varhash->{$key});
