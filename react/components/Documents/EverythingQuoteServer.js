...
/**
 * Parse a quote string into quote text and attribution
 * Quotes are formatted as: "Quote text<br><br>--Attribution" or just "Quote text"
 *
 * @param {string} rawQuote - Raw quote string with <br> tags and optional attribution
 * @returns {Object} { text: string, attribution: string|null }
 */
const parseQuote = (rawQuote) => {
  // Split on <br><br>-- or <br><br> -- (with space) or <br><BR>--
  const attributionMatch = rawQuote.match(/^(.*?)<br\s*\/?\s*><br\s*\/?\s*>(?:--\s*|<em>|<i>)(.*?)$/i)

  if (attributionMatch) {
    const text = attributionMatch[1].trim()
    let attribution = attributionMatch[2].trim()

    // Clean up attribution - remove trailing </em> or </i> tags
    attribution = attribution.replace(/<\/(em|i)>\s*$/i, '')

    return { text, attribution }
  }

  // No attribution found - entire string is the quote
  return { text: rawQuote.trim(), attribution: null }
}

// Quote data extracted from legacy.js (originally 250+ quotes)
// These quotes use E2 link markup like [node title] which ParseLinks will convert to links
// Format: "Quote text<br><br>--Attribution" or just "Quote text"
const RAW_QUOTES = [
 ...
 * on every page load. React version lazy-loads only when the page is visited.
 */
const EverythingQuoteServer = () => {
  const [currentQuote, setCurrentQuote] = useState(null)

  const getRandomQuote = () => {
    const randomIndex = Math.floor(Math.random() * RAW_QUOTES.length)
    return parseQuote(RAW_QUOTES[randomIndex])
  }
  ...
  // Replace <br> and <br/> tags with actual line breaks for display
  const formatTextWithLineBreaks = (text) => {
    return text.split(/<br\s*\/?\s*>/gi).map((line, i) => (
      <React.Fragment key={i}>
        {i > 0 && <br />}
        <ParseLinks text={line} />
      </React.Fragment>
    ))
  }
...
export default EverythingQuoteServer
