...
BEGIN
{
	foreach my $sub qw(getRandomNode handle_errors query_vars_string tagApprove htmlScreen cleanupHTML tableWellFormed debugTag debugTable screenTable buildTable breakTags encodeHTML decodeHTML htmlFormatErr htmlErrorUsers htmlErrorGods jsWindow urlGen getCode getPages getPageForType getPage rewriteCleanEscape urlGenNoParams linkNode linkNodeTitle nodeName evalCode htmlcode embedCode parseCode listCode quote insertNodelet updateNodelet genContainer containHtml displayPage gotoNode confirmUser parseLinks urlDecode loginUser getCGI getTheme printHeader handleUserRequest cleanNodeName clearGlobals opNuke opLogin opLogout opNew getOpCode execOpCode isSuspended mod_perlInit escapeAngleBrackets showPartialDiff showCompleteDiff generate_test_cookie assign_test_condition check_test_substitutions recordUserAction processVarsSet)
	{
