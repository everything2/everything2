package Everything::Page::usergroup_picks;
...
sub buildReactData {
    ...
    return { type => 'usergroup_picks', error => 'Configuration not found' }
    ...
    # Build list of usergroups with counts
    my @groups = ();
    foreach my $node_id (keys %$webloggables) {
        # Skip certain node IDs (as in original)
        next if ($node_id == 165580 || $node_id == 923653 || $node_id == 114);

        my $group_node = $DB->getNodeById($node_id);
    ...
    return {
        ...
        viewGroupName   => $group_node->{title},
        entries         => \@entries,
